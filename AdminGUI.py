...
def AdminPortal():
    ...
    def button1Click():
        ...
        def addNew():
            ...
            guiLabel = tk.Label(frame, text="Enter Room Details (Seperate by commas and spaces *no brackets*)", font=('Helvetica', '14'))
            guiLabel.pack()
            guiEntry = tk.Entry(frame, font=('Helvetica', '14'), width=30)
            guiEntry.pack(padx=40)
            #if the user clicks submit, send the query along
            def dingl():
                userInput = guiEntry.get()
                SQL.addSomething("Rooms (Name, Capacity, Type) VALUES ({});".format(userInput))
                reset()
    ...
    def button2Click():
        ...
        def addNew():
            ...
            if addCounter == True:
                return
            button7.config(foreground='white', background='#E59989')
            guiLabel = tk.Label(frame, text="Enter New Equipment Details", font=('Helvetica', '14'))
            guiLabel.pack()
            guiEntry = tk.Entry(frame, font=('Helvetica', '14'), width=30)
            guiEntry.pack(padx=40)
            ...
            def dingl():
                userInput = guiEntry.get()
                SQL.addSomething("Equipment (Name, Type, PurchaseDate, Condition, RoomNumber, LastMaintenance) VALUES ({});".format(userInput))
                #resets view
                reset()
            button8 = tk.Button(frame, text="Submit", command=dingl, height=1, width=8, font=('Helvetica', '12'), bg='#9389E5')
            button8.pack()
    ...
    def button3Click():
        ...
        def addNew():
            ...
            if addCounter == True:
                return
            button7.config(foreground='white', background='#E59989')
            guiLabel = tk.Label(frame, text="Enter Fitness Class Details", font=('Helvetica', '14'))
            guiLabel.pack()
            guiEntry = tk.Entry(frame, font=('Helvetica', '14'), width=30)
            guiEntry.pack(padx=40)
            ...
            def dingl():
                userInput = guiEntry.get()
                SQL.addSomething("FitnessClass (ClassName, TrainerID, RoomNumber, ClassDate, SessionTime, EndTime, Cost, Capacity) VALUES ({});".format(userInput))
                reset()
                guiLabel.pack_forget()
                guiEntry.pack_forget()
                button8.pack_forget()
                #removes text from textbox
                guiEntry.delete(0, tk.END)
            button8 = tk.Button(frame, text="Submit", command=dingl, height=1, width=8, font=('Helvetica', '12'), bg='#9389E5')
            button8.pack()
            addCounter = True
        ...
        def update():
            ...
            guiLabel = tk.Label(frame, text="Enter Fitness Class Change", font=('Helvetica', '14'))
            guiLabel.pack()
            entries = []
    ...
    def button4Click():
        ...
        def collectFees():
            def confirm():
                amount = str(guiEntry.get()).replace("$","").replace(" ", "")
                ...
                returnButton()
            guiEntry = tk.Entry(frame, font=('Helvetica', '14'), width=15)
            guiEntry.pack(padx=30, pady=15)
            guiEntry.insert(0, "Enter Amount")
