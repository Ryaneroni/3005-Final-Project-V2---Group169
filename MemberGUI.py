...
def MemberPortal(e):
    ...
    def button2Click():
        ...
        def update():
            ...
            updateCounter = True
            button777.config(foreground='white', background='#9389E5')
            newLabel = tk.Label(frame, text="Enter Health / Fitness Item Change", font=('Helvetica', '14'))
            newLabel.pack()
            newLabel = tk.Label(frame, text="*To avoid errors, click on the element you want to change only once*", font=('Helvetica', '14'))
            newLabel.pack()
            newLabel = tk.Label(frame, text="*If you are encountering errors click return and come back*", font=('Helvetica', '14'))
            newLabel.pack()
            entries = []
            #Creates the fields to check
            def dingl():
        ...
        def onSelect2(event):
            ...
            if current_selection:  # Check if there's any selection
                currentIndex = current_selection[0]
                if currentIndex == 0 or currentIndex == 7:  # If the first item or 6th
        ...
        listbox3.bind('<<ListboxSelect>>', onSelect3)  
        def viewRoutine():
            ...
            def updateRoutine():
                def sendUpdate():
                    ...
                    returnButton()
                guientry = tk.Entry(frame, font=('Helvetica', '14'), width=60)
                guientry.pack(padx=30, pady=15)
                guientry.insert(0, "Enter Routine")
    ...
    def button3Click():
        ...
        guilabel2 = tk.Label(frame, text="Your Private Sessions", font=('Helvetica', '14'))
        guilabel3 = tk.Label(frame, text="Class Browser (Classes you are not in)", font=('Helvetica', '14'))
        frame.pack(padx=40, pady=20, fill=tk.BOTH, expand=True)
        listbox2 = tk.Listbox(frame, font=('Helvetica', '15'))
        listbox2.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        listbox3 = tk.Listbox(frame, font=('Helvetica', '15'))
        ...
        def addFClass():
            ...
            def getClass():
                ...
                selectedItem = listbox2.get(index)
                temp = (str(selectedItem).split(","))
                temp = temp[-2].replace("\'", "").replace(",", "").replace("$", "").replace(" ", "")
                #if its a bad index return
                if((index) == (0,) or (index) == (1,) or (index) == (2,) or (index) == (3,) or selectedItem == 'No Registered Classes' or selectedItem ==''):
                    print("bad index")
                    return
                #get the classID
                selectedItem = str(selectedItem).split(",")
                classID = selectedItem[0].replace("(", "")
        ...
        def withdrawFClass():
                def deleteClass():
                    ...
                    if((index) == (0,) or selectedItem == 'No Registered Classes' or selectedItem ==''):
                        print("bad index")
                        return
                    selectedItem = str(selectedItem).split(",")
                    classID = selectedItem[0].replace("(", "")
        ...
        def viewPrivate():
            ...
            def withdrawPClass():
                def deleteClass():
                    ...
                    selectedItem = listbox3.get(index)
                    temp = (str(selectedItem).split(","))
                    temp = temp[-1].replace("\'", "").replace(",", "").replace("$", "").replace(")", "").replace(" ", "")
                    if((index) == (0,) or selectedItem == 'No Registered Sessions' or selectedItem ==''):
                        print("bad index")
                        return
            ...
            def createPrivSesh():
                ...
                newLabel = tk.Label(frame, text="Enter Date as yyyy-mm-dd", font=('Helvetica', '14'))
                newLabel.pack()
                ...
                def dingl():
                    ...
                    def selectTrainer():
                        ...
                        if selectedItem == '':
                            return
                        trainerID = str(selectedItem).split(",")
